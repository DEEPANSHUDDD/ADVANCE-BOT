...
async def clone_repo(client, message):
    ...
    try:
        ...
        print(f"Clone result: {result.stdout} {result.stderr}")
...
async def commit_changes(client, message):
    ...
    try:
        print(f"Committing changes in repository: {repo_path} with message: {commit_message}")
        os.chdir(repo_path)
...
async def push_changes(client, message):
    ...
    try:
        print(f"Pushing changes in repository: {repo_path}")
        os.chdir(repo_path)
...
async def pull_changes(client, message):
    repo_path = message.text.split(' ', 1)[1]
    try:
        print(f"Pulling changes in repository: {repo_path}")
        os.chdir(repo_path)
        result = subprocess.run(['git', 'pull'], capture_output=True, text=True)
        print(f"Pull result: {result.stdout} {result.stderr}")
        os.chdir('..')
...
async def view_file(client, message):
    ...
    try:
        print(f"Viewing file in repository: {repo} at path: {path}")
...
async def edit_file(client, message):
    ...
    try:
        print(f"Editing file in repository: {repo} at path: {path}")
        # Get the file's SHA
        response = requests.get(url, headers=HEADERS)
        print(f"GitHub API response: {response.status_code} {response.json()}")
        if response.status_code != 200:
...
async def add_file(client, message):
    ...
    try:
        print(f"Adding file to repository: {repo} at path: {path}")
        data = {
...
async def remove_file(client, message):
    ...
    try:
        print(f"Removing file from repository: {repo} at path: {path}")
        # Get the file's SHA
        response = requests.get(url, headers=HEADERS)
        print(f"GitHub API response: {response.status_code} {response.json()}")
        if response.status_code != 200:
